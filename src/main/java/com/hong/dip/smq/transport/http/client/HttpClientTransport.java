...
public class HttpClientTransport extends ServiceSupport implements ClientTransport {
	...
	@Override
	public void startMessageSender(Node node, QueueStorage queue) {
		HttpMessageSender sender = new HttpMessageSender(node, queue);
	...
	class HttpMessageSender extends Thread{
		...
		public HttpMessageSender(Node node, QueueStorage storage) {
			super("Http-Message-Sender " + storage.getName());
			this.storage = storage;
			httpClient = clientManager.createHttpClient(getURL(node, storage));
			buffer = ByteBuffer.allocateDirect(storage.getChunkSize());
		}

		private String getURL(Node node2, QueueStorage storage2) {
			return "http://"+node2.getIp()+ ":" + node2.getPort() +"/queue/" + storage2.getName();
		}
				...
				if(chunkIdx >= 0){
					...
					try{
						...
						while((buffer = reader.readChunk(this.buffer)) != null){
							MsgPostCmd postCmd = createMsgPostCmd(msg, allParts, partIdx, reader, buffer);
		...
		private MsgPostCmd createMsgPostCmd(MessageStorage msg, List<ChunkableDataSource> allParts, int partIdx,
				ChunkReader reader, ByteBuffer buffer) throws IOException {
			MsgPostCmd postCmd = new MsgPostCmd(this.storage.getName(), msg.getID(), allParts.size(), partIdx, reader.getSize(), buffer.remaining());
			postCmd.setAttachmentNames(((FlumeMessageStorage)msg).getAttachmentNames());
			//postCmd.setAttachmentPaths(((FlumeMessageStorage)msg).getAttachmentPath());
			return postCmd;
		}
